...
Conformément aux consignes du projet, nous confrontons explicitement les résultats de Ge et al.~\cite{ge2021ota} à ceux produits par notre implémentation. Le tableau~\ref{tab:compare} distingue les métriques \emph{reproduites ou validées} par notre code de celles qui restent des \emph{références bibliographiques} de l'article (nous ne les ré-inventons pas, nous les citons pour situer notre travail).
...
Validation fidèle de l'\emph{algorithme} d'assignation (Sinkhorn, coût, Dynamic~$k$, Center Prior) \\
\addlinespace
Jeux de données &
MS COCO~\cite{lin2014coco}, CrowdHuman~\cite{shao2018crowdhuman} &
Tests numériques aléatoires + scénario synthétique (2 objets qui se chevauchent, 625 ancres) \\
\addlinespace
AP COCO (FCOS, ResNet-50, 1$\times$) &
\textbf{40,7\,\%} AP (val., Tableau~1) &
Non mesuré --- hors périmètre de ce projet (pas d'entraînement du détecteur complet) \\
\addlinespace
AP COCO (ResNet-101-FPN) &
\textbf{45,3\,\%} AP (test-dev, Tableau~5) &
Référence bibliographique uniquement \\
\addlinespace
CrowdHuman (MR) &
\textbf{46,6\,\%} MR (Tableau~6) &
Non mesuré --- référence bibliographique \\
\addlinespace
Validation Sinkhorn &
$\gamma=0.1$, $T=50$ itér. (choix empirique) &
Écart au LP exact~: \textbf{0,16--0,73\,\%} ; résidu marginal $<10^{-5}$ (section~\ref{sec:experimentations}) \\
\addlinespace
Gestion des ancres ambiguës &
Moins d'ambiguïtés qu'avec Min Area / Max IoU / Min Loss (Tableau~3, Fig.~3) &
Comportement \emph{qualitatif} cohérent~: OTA détecte des ambiguïtés «~douces~» ($\max_i\pi^\star_{ij}<0.9$) là où les heuristiques en comptent presque zéro (tab.~\ref{tab:ambig}, $r=15$) \\
\addlinespace
Implémentation &
PyTorch, intégration dans pipeline FCOS &
Python/NumPy/SciPy, tests unitaires, \texttt{demo.py}, dépôt GitHub public \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Lecture.} Notre comparaison avec l'article est donc \textbf{partielle mais explicite}~: nous confirmons numériquement le cœur algorithmique (Sinkhorn-Knopp, paramètres $\gamma=0.1$) et reproduisons qualitativement le phénomène central des ancres ambiguës ; nous rapportons les gains d'AP de l'article comme benchmark de référence, sans prétendre les avoir re-mesurés dans ce projet.
